...
//	private $conn;
	...
	function createChart ($valAr, $u = null, $checkWhenNotAID = false) {
		global $config;
		...
		} else if ($checkWhenNotAID) { // check by name
			$query = "SELECT * FROM " . $this->table_name . " WHERE name = ? LIMIT 0,1";	
			$stmt = $this->conn->prepare($query);
			$stmt->bindParam(1, $valAr['name']);
			$stmt->execute();
			$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		$adstmt = $this->conn->prepare("SELECT id FROM " . $this->table_name . " WHERE uid = ".$u);
		$adstmt->execute();
		...
		if ($stmt->execute()) {
			$ar = array('link' => $this->cLink.'/'.$valAr['uname'].'/'.$nc);
			return $ar;
		} else return false;
	...
	<div class="form-group apply-data">
		...
		<div class="clearfix"></div>
			...
			<? }
		} else echo '<div class="alerts alert-warning">You must login to access your saved mAstro_data</div>'; ?>
