<?php
class Relationship extends Config {

	// database connection and table name
//	private $conn;
	private $table_name = "relationship";
	...
	function createChart ($valAr, $replaceAID) {
		global $config;
		...
		if ($astro_data_id) {
			$query = "SELECT * FROM " . $this->table_name . " WHERE aid = ? OR aid = ? LIMIT 0,1";	
			$stmt = $this->conn->prepare($query);
			$stmt->bindParam(1, $astro_data_id);
			$stmt->bindParam(2, $replaceAID);
			$stmt->execute();
		...
		$stmt = $this->conn->prepare($query);
		$stmt->bindParam(1, $nc);
		for ($i = 0; $i < count($vAr); $i++) $stmt->bindParam($i+2, $vAr[$i]);

		if ($stmt->execute()) {
			$ar = array('link' => $this->rLink.'/'.$valAr['uname'].'/'.$nc);
			return $ar;
		} else return false;
	...
	function showForm () {
		...
		if (!$this->u) echo '<div class="alerts alert-warning">You must login to access your saved mAstro_data</div>';
...
<div class="col-lg-6 chart-inputs">
<?	if ($total_dList > 0 || count($dOList) > 0) { ?>
	<div class="form-group apply-data">
		...
		<div class="col-lg-7 no-padding-right">
			<select class="chosen-select form-control" name="aid<? echo $q ?>" id="apply_data">
...
<? 		}
	...
	function readAll ($u, $or = '', $page = '', $from_record_num = '', $records_per_page = '') {
		...
		while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
			$row['link'] = $this->rLink.'/'.$row['uname'].'/'.$row['n'];
...
//		$row['content'] = content($row['content']);
		...
		$this->uid = $row['uid'];
		$this->link = $row['link'] = $this->rLink.'/'.$row['uname'].'/'.$row['n'];
		$row['stt'] = (int)$row['stt'];
