...
if (get_magic_quotes_gpc()) {
	$longitude = unserialize(stripslashes($ser_L1));
	$hc = unserialize(stripslashes($ser_hc1));
} else {
	$longitude = unserialize($ser_L1);
	$hc = unserialize($ser_hc1);
}
...
//	display_house_cusp_number($i, -$angle, $diameter - $inner_diameter_offset * 5.1, $xy);
	...
	imagettftext($im, 9, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $black, __ASTRO.'/arial.ttf', $i);
}
...
drawboldtext($im, 10, 0, $x1 + $center_pt - 2, $y1 + $center_pt - 2, $dark, __ASTRO.'/arial.ttf', 'AC', 0);
drawboldtext($im, 10, 0, -$x1 + $center_pt - 14, $y1 + $center_pt - 2, $dark, __ASTRO.'/arial.ttf', 'DC', 0);
...
drawboldtext($im, 10, 0, $x2 + $center_pt - 2, $y2 + $center_pt - 2, $dark, __ASTRO.'/arial.ttf', 'MC', 0);
drawboldtext($im, 10, 0, -$x2 + $center_pt - 12, -$y2 + $center_pt + 10, $dark, __ASTRO.'/arial.ttf', 'IC', 0);
...
for ($i = 1; $i <= 12; $i++) {
	...
	if ($i == 1 Or $i == 5 Or $i == 9) 
		...
		$clr_to_use = $blue;
	drawboldtext($im, 16, 0, $x1 + $center_pt, $y1 + $center_pt, $clr_to_use, __ASTRO.'/HamburgSymbols.ttf', chr($sign_glyph[$i]), 0);
...
//	else display_planet_glyph($angle_to_use, $radius - ($dist_from_diameter2), $xy);
	...
	imagettftext($im, 14, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $planet_color, __ASTRO.'/HamburgSymbols.ttf', chr($pl_glyph[$sort_pos[$i]]));
