...
for ($i = $num_planets - 1; $i >= 0; $i--) {
	...
	if ($flag == False) display_planet_glyph2($angle_to_use, $radius - $dist_from_diameter2, $xy);
	else display_planet_glyph2($angle_to_use, $radius - ($dist_from_diameter2b), $xy);
	$xx = ($xy[0] + $center_pt);
	$yy = ($xy[1] + $center_pt);

	imagettftext($im, 16, 0, $xx, $yy, $pinkred, 'HamburgSymbols.ttf', chr($pl_glyph[$sort_pos2[$i]]));
...
/*	if ($flag == False) {
	...
	imageline($im, $x1 + $center_pt, $y1 + $center_pt, $x2 + $center_pt, $y2 + $center_pt, $pink);
