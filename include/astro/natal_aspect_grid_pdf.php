...
if (get_magic_quotes_gpc()) {
	...
	$hc1 = unserialize(stripslashes($ser_hc1));
} else {
	$longitude = unserialize($ser_L1);
	$hc1 = unserialize($ser_hc1);
}
...
for ($i = 0; $i <= $last_planet_num - 1; $i++) imageline($im, $margins, $cell_height * ($i + 1), $margins + $cell_width * ($i + 1), $cell_height * ($i + 1), $black);
...
for ($i = 1; $i <= $last_planet_num; $i++) imageline($im, $margins + $cell_width * $i, $cell_height * $num_planets, $margins + $cell_width * $i, $cell_height * $i, $black);
...
for ($i = 0; $i <= $last_planet_num - 1; $i++) {
	for ($j = $i + 1; $j <= $last_planet_num; $j++) {
		...
		if ($q > 0) drawboldtext($im, 14, 0, $margins + $cell_width * ($i + 0.15), $cell_height * ($j + 1 - 0.20), $asp_color[$q], 'HamburgSymbols.ttf', chr($asp_glyph[$q]), 0);
	}
}
