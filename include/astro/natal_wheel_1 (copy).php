...
//	if ($i != 1 And $i != 10) 
	...
	imagettftext($im, 7, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $black, 'arial.ttf', $i);
}
...
$cnt = 0;
for ($i = $offset_from_start_of_sign; $i <= $offset_from_start_of_sign + 359; $i = $i + 4) {
	$x1 = -$radius * cos(deg2rad($i));
	$y1 = -$radius * sin(deg2rad($i));
...
for ($i = 0; $i <= $last_planet_num - 1; $i++) {
	$x1 = (-$radius + $dist_from_diameter1a * 7.2) * cos($angle_to_use);
	$y1 = ($radius - $dist_from_diameter1a * 7.2) * sin($angle_to_use);
