...
for ($i = 0; $i <= $last_planet_num + 1; $i++) imageline($im, $margins, $cell_height * ($i + 1), $margins + $cell_width * $num_planets, $cell_height * ($i + 1), $black);

// draw the grid - vertical lines
for ($i = 0; $i <= $last_planet_num + 1; $i++) imageline($im, $margins + $cell_width * $i, $cell_height * ($num_planets + 1), $margins + $cell_width * $i, $cell_height, $black);
...
for ($i = 0; $i <= $last_planet_num; $i++) {
	...
	if ($i <= $last_planet_num) drawboldtext($im, 16, 0, $margins + $left_margin_planet_table, $cell_height + $cell_height * ($i + 1), $red, 'HamburgSymbols.ttf', chr($pl_glyph[$i]), 0);
...
for ($i = 0; $i <= $last_planet_num; $i++) {
	for ($j = 0; $j <= $last_planet_num; $j++) {
		...
		if ($i == 0 Or $i == 1 Or $j == 0 Or $j == 1) $orb = 1;
		else $orb = 1;
		...
		if ($q > 0 And $j != $last_planet_num - 2) drawboldtext($im, 14, 0, $margins + $cell_width * ($i + 0.15), $cell_height + $cell_height * ($j + 1 - 0.20), $asp_color[$q], 'HamburgSymbols.ttf', chr($asp_glyph[$q]), 0);
	}
}
