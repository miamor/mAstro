...
$planet_color2 = $lavender;
$planet_color3 = $pinkred;
...
for ($i = 1; $i <= 12; $i = $i + 1) {
	...
	if ($i != 1 And $i != 10) imageline($im, $x1 + $center_pt, $y1 + $center_pt, $x2 + $center_pt, $y2 + $center_pt, $black);
...
imageline($im, $x1 + $center_pt, $y1 + $center_pt, $x2 + $center_pt, $y2 + $center_pt, $dark);
$x1-=24; $y1+=7;
drawboldtext($im, 16, 0, $x1 + $center_pt, $y1 + $center_pt, $dark, 'HamburgSymbols.ttf', chr($pl_glyph[15]), 1);
...
imageline($im, $x1 + $center_pt, $y1 + $center_pt, $x2 + $center_pt, $y2 + $center_pt, $dark);
$x2-=6; $y2-=6;
drawboldtext($im, 16, 0, $x2 + $center_pt, $y2 + $center_pt, $dark, 'HamburgSymbols.ttf', chr($pl_glyph[16]), 1);
...
for ($i = 1; $i <= 12; $i++) {
	...
	if ($i == 1 Or $i == 5 Or $i == 9) $clr_to_use = $red;
	else if ($i == 2 Or $i == 6 Or $i == 10) $clr_to_use = $green;
	else if ($i == 3 Or $i == 7 Or $i == 11) $clr_to_use = $orange;
	else if ($i == 4 Or $i == 8 Or $i == 12) $clr_to_use = $blue;
...
for ($i = $num_planets - 1; $i >= 0; $i--) {
	...
	if ($flag == False) display_planet_glyph($angle_to_use, $radius - $dist_from_diameter1, $xy);
	else display_planet_glyph($angle_to_use, $radius - ($dist_from_diameter1b), $xy);
...
for ($i = $num_planets - 1; $i >= 0; $i--) {
	...
	if ($flag == False) display_planet_glyph($angle_to_use, $radius - $dist_from_diameter2, $xy);
	else display_planet_glyph($angle_to_use, $radius - ($dist_from_diameter2b), $xy);
	...
	imageline($im, $x1 + $center_pt, $y1 + $center_pt, $x2 + $center_pt, $y2 + $center_pt, $planet_color2);
...
for ($i = $num_planets - 1 - 4; $i >= 0; $i--) {
	if ($sort_pos3[$i] > SE_TNODE) continue;
	...
	if ($flag == False) display_planet_glyph($angle_to_use, $radius - $dist_from_diameter3, $xy);
	else display_planet_glyph($angle_to_use, $radius - ($dist_from_diameter3b), $xy);
	...
	imageline($im, $x1 + $center_pt, $y1 + $center_pt, $x2 + $center_pt, $y2 + $center_pt, $planet_color3);
...
for ($i = 0; $i <= $last_planet_num; $i++) {
	for ($j = 0; $j <= $last_planet_num - 4; $j++) {
		...
		if ($sort_pos1[$i] == 0 Or $sort_pos1[$i] == 1 Or $sort_pos3[$j] == 0 Or $sort_pos3[$j] == 1) $orb = 2;
		else $orb = 2;
		...
		if ($q > 0) {
			if ($q == 1 Or $q == 3 Or $q == 6) $aspect_color = $green;
			else if ($q == 4 Or $q == 2) $aspect_color = $red;
			else if ($q == 5) $aspect_color = $blue;
