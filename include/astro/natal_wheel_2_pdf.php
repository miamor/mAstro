...
// MUST BE HERE - I DO NOT KNOW WHY - MAYBE TO PRIME THE PUMP
  imagettftext($im, 10, 0, 0, 0, $black, __ASTRO.'/arial.ttf', " ");
...
// draw in the actual house cusp numbers and sign
  for ($i = 1; $i <= 12; $i = $i + 1)
  {
    ...
    imagettftext($im, 14, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $clr_to_use, __ASTRO.'/HamburgSymbols.ttf', chr($sign_glyph[$sign_pos]));
    ...
    imagettftext($im, 10, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $black, __ASTRO.'/arial.ttf', $t . chr(176));
    ...
    imagettftext($im, 10, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $black, __ASTRO.'/arial.ttf', $t . chr(39));
  }
...
// draw the lines for the house cusps
  ...
  for ($i = 1; $i <= 12; $i = $i + 1)
  {
    ...
    imagettftext($im, 10, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $black, __ASTRO.'/arial.ttf', $i);
  }
...
// put planets in chartwheel
  ...
  //for ($i = 2; $i >= 0; $i--)
  {
    ...
    imagettftext($im, 16, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $planet_color, __ASTRO.'/HamburgSymbols.ttf', chr($pl_glyph[$sort_pos[$i]]));
    ...
    $t = $int_reduced_pos;
    if ($int_reduced_pos < 10) { $t = "0" . $int_reduced_pos; }

    display_planet_glyph($our_angle, $angle_to_use, $radius - $dist_from_diameter1 - 20, $xy, 1);
    imagettftext($im, 10, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $planet_color, __ASTRO.'/arial.ttf', $t . chr(176));
    ...
    imagettftext($im, 10, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $clr_to_use, __ASTRO.'/HamburgSymbols.ttf', chr($sign_glyph[$sign_pos]));
    ...
    imagettftext($im, 10, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $planet_color, __ASTRO.'/arial.ttf', $t . chr(39));
    ...
    if (strtoupper(mid($retrograde, $sort_pos[$i] + 1, 1)) == "R")
    {
      ...
      imagettftext($im, 10, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $red, __ASTRO.'/HamburgSymbols.ttf', chr(62));
    }
  }
...
// draw in the aspect lines
  for ($i = 0; $i <= $last_planet_num - 1; $i++)
  {
    ...
    {
      if ($sort_pos[$i] >= SE_LILITH Or $sort_pos[$j] >= SE_LILITH) { continue; }
      ...
      $orb = 6;
      if ($sort_pos[$i] == SE_SUN Or $sort_pos[$i] == SE_MOON Or $sort_pos[$j] == SE_SUN Or $sort_pos[$j] == SE_MOON) { $orb = 8; }
  ...
  imagepng($im, $chartwheel_filename);          //output image to a file
  imagedestroy($im);
