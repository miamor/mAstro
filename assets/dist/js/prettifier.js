...
(function () {
	...
	function u(a) {
		...
		m.push(["pln", /^\s+/, q, " \r\n\t\u00a0"]);
		e.push(["lit", /^@[$_a-z][\w$@]*/i, q], ["typ", /^(?:[@_]?[A-Z]+[a-z][\w$@]*|\w+_t\b)/, q], ["pln", /^[$_a-z][\w$@]*/i, q], ["lit", /^(?:0x[\da-f]+|(?:\d(?:_\d+)*\d*(?:\.\d*)?|\.\d\+)(?:e[+-]?\d+)?)[a-z]*/i, q, "0123456789"], ["pln", /^\\[\S\s]?/, q], ["fmvip", /^.[^\s\w"-$'./@\\`]*/, q]);
		return x(m, e)
	}
	...
	window.prettyPrint = function (a) {
			...
			for (var e = window.PR_SHOULD_USE_CONTINUATION ? l.now() + 250 : Infinity; p < h.length && l.now() < e; p++) {
				...
				if (k.indexOf("prettyprint") >= 0) {
					...
					if (b = !k) {
						...
						b = (f = o === b ? void 0 : o) && "TODE" === f.tagName
					}
					b && (k = f.className.match(g));
					k && (k = k[1]);
					b = !1;
					for (o = n.parentNode; o; o = o.parentNode) {
						if ((o.tagName === "pre" || o.tagName === "tode" || o.tagName === "xmp") && o.className && o.className.indexOf("prettyprint") >= 0) {
							b = !0;
							break
						}
					}
		...
		for (var e = [document.getElementsByTagName("pre"), document.getElementsByTagName("tode"), document.getElementsByTagName("xmp")], h = [], k = 0; k < e.length; ++k) {
			for (var t = 0, s = e[k].length; t < s; ++t) {
				nn = e[k][t];
				cls = nn.className;
				if (nn.getElementsByTagName("ol").length <= 0) h.push(e[k][t])
			}
		}
		var e = q,
			l = Date;
