	...
	if (Messages) {
		...
		$.each(arr, function(i, val) { // Duyệt qua từng tin
				...
				if (indexUser !== -1) { // Nếu có nickname của thành viên đang truy cập trong danh sách
					...
					if (!$tabPrivate.length) { // Nếu chưa có mục chat riêng thì tạo mới
						...
						$tabPrivate = $("<div>", {
							...
							html: '<h3><a href="' + MAIN_URL+'/u/'+chat_name + '" onclick="return false">@' + chat_name + '</a></h3><span class="chatbox-change-mess"></span>'
						...
						$tabPrivate.hide();
						if (my_getcookie('chatbox_active') !== 'publish') $('.chatbox-change[data-id="publish"]').click();
					}
			...
			if (messText === "/buzz") { // Nếu có ký hiệu buzz
				$msg.html('<img src="'+MAIN_URL+'/assets/plugins/mchat/img/buzz.gif" width="62" height="16" />'); // Thay bằng ảnh buzz
		...
		//	icons();
			...
			$(".chatbox-change > h3").each(function() { // Duyệt qua từng tab riêng
				...
				if ($this.text().indexOf('@') !== -1) {
					var $status = userOnline($this.text()).closest("ul").prev("h4"),
					clas;
...
$('#chatbox-header').click(function () {
	$('#chatbox-main').toggleClass('restrict');
	my_setcookie("chatbox_restrict", $('#chatbox-main.restrict').length, false); // Lưu cookie
});
/*
var clicked = false;
$('.chatbox-change').one('click', function () {
	clicked = true;
});
$('.chatbox-change').on('click', function () {
	if (clicked == true) {
...
console.log(clicked);
console.log(my_getcookie("chatbox_restrict"));
*/
